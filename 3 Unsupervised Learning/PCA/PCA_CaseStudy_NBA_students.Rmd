...
```{r, echo=FALSE}
setwd("~/OneDrive - Universidad Carlos III de Madrid/U/Clases/Segundo/Semestre 1/Stat Learning/Reducida/3 Unsupervised Learning/Case Study sports analytics (NBA)")
...
```{r}
# To know how many missing values for each player
...
We remove some variable as they are redundant: 
 ...
 - PTS, because PTS = FT + 2 * X2P + 3 * X3P (these are points per game)
...
```{r}
# Enter your code here to plot the boxplots and scaled boxplots
boxplot(scale(nba), col = "salmon")
...
```{r}
# Enter your code here to display a correlation graph
plot(scale(nba), col = "lightblue")
...
```{r}
pca = prcomp(nba, scale=T) # this principal component analysis
...
```{r}
screeplot(pca,main="Screeplot",col="yellowgreen",type="barplot",pch=19)
```
...
barplot(pca$rotation[,1], las=2, col="palevioletred4")
```
...
```{r}
fviz_contrib(pca, choice = "var", axes = 1) # Line = average of contribution
```
...
```{r}
barplot(pca$rotation[,2], las=2, col="lightgreen")
```
...
```{r}
# correlation in the unit circle, how it contributes to the variables, Color and length.
