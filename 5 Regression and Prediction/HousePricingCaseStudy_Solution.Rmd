...
leaflet() %>% 
  ...
  setView(lng=median(Sacramento$longitude), lat=median(Sacramento$latitude), zoom = 10) %>%
  ...
  addCircles(data=testing,
             ...
             color = "red") %>%
  addLabelOnlyMarkers(
    lng = median(Sacramento$longitude)-.6, lat = median(Sacramento$latitude)+.3,
    label = "Train vs Test sets",
    labelOptions = labelOptions(textsize = "20px",noHide = T, textOnly = T))
```

Training in Blue, Testing in Red

Seems a reasonable partition (no bias)

Now, for the training set, include the price and sqft information
...
# Some simple and multiple regression models
...
R2 is roughly 50%, the size is highly significant
...
A better simple regression

```{r}
# Insert your code here:

linFit <- lm(log(price) ~ log(sqft), data=training)
summary(linFit)
```
...
cor(testing$price, pr.simple)^2
```

Not a bad model using only one regressor
